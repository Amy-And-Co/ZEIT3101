defmodule AmyandcoWeb.PublicRoomLive do
  use AmyandcoWeb, :live_view
  require Logger
# This is the public chat module code, which handles the unauthenticated users chat
# Declares all initial variables for the chat client
  @impl true
...
# Handles an event when the user submits a message, triggers a new message event
  @impl true
  ...
  # Handles a new-message event, when someone is the chat sends a message
  @impl true
  def handle_info(%{event: "new-message", payload: message}, socket) do
    {:noreply, assign(socket, messages: [message])}
  end
  # Changes held chat messages values when text box is changed
  @impl true
  def handle_event("form_update", %{"chat" => %{"message" =>message}}, socket) do
    {:noreply, assign(socket, message: message)}
  end

  # Handles an event when the chat's presence is changed
  @impl true
  ...
  # Displays the chat message with styling based on who sent it
  ...
  # Changes stylesheet for chat colour changes when event is triggered
  @impl true
  def handle_event("change_color", %{"value" =>color}, socket) do
    ...
    # Prints HTML code for chat colour changes
